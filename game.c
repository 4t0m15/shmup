...
void InitializeGameVariables(GameState* gameState) {
    ...
    InitParticleSystem(&gameState->effects);
    InitAudioSystem(&gameState->audio);
    InitAllAudioAssets(&gameState->audio);
    InitPowerUpSystem(&gameState->powerups);
    InitBalanceSystem(&gameState->balance);
    InitQoLSystem(&gameState->qol);
...
void InitGame(GameState* gameState) {
    ...
    // Start menu music
    PlayMusicTrack(&gameState->audio, MUSIC_MENU);

    // Load achievements and weapon progression
    LoadAchievements(&gameState->achievements);
    // (If you have weapon progression save/load, call it here)
}
...
static void UpdateGamePlaying(GameState* gameState, float delta) {
    ...
    // --- Remove old bullet update loop (handled by UpdateAdvancedBullets) ---
    ...
    if (CheckEnemyBulletPlayerCollisions(gameState)) {
        ...
        CreateHitEffect(&gameState->effects, player_center, false);
...
static void DrawGamePlaying(const GameState* gameState) {
    ...
    DrawAdvancedBullets(gameState); // Use advanced bullets
